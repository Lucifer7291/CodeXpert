...
function QuizCard({ quiz, index }) {
  return (
    ...
    >
      {/* Card header */}
      <div className={`relative h-32 bg-gradient-to-br ${quiz.grad}
                flex items-center justify-center flex-shrink-0`}>
        <span className="text-5xl select-none">{quiz.emoji}</span>

        {/* Difficulty tag */}
        <span className={`absolute top-3 left-3 px-2.5 py-0.5 rounded-full
                    text-[10px] font-black tracking-wide ${quiz.tagColor}`}>
          {quiz.difficulty}
        </span>
...
function Quiz() {
  ...
  const { page, filters, quizzes } = data;
  ...
  const filtered = useMemo(() => {
    return quizzes.filter((q) => {
      ...
      const matchDifficulty = difficulty === "All Levels" || q.difficulty === difficulty;
      return matchSearch && matchCategory && matchDifficulty;
    });
  }, [quizzes, search, category, difficulty]);

  useEffect(() => { setCurrent(1); }, [search, category, difficulty]);
      ...
      <div ref={topRef} className="max-w-6xl mx-auto px-6 py-10">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {(search || category !== "All" || difficulty !== "All Levels") && (
            <button
              onClick={() => { setSearch(""); setCategory("All"); setDifficulty("All Levels"); }}
        ...
        {paginated.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <h3 className="font-black text-lg mb-2">No quizzes found</h3>
