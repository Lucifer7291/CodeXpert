...
// ─── Format icon ──────────────────────────────────────────────────────────────
const formatIcon = { PDF: "📄", Markdown: "📋", Cheatsheet: "⚡" };
...
function NoteCard({ note, index }) {
  return (
    ...
    >
      {/* Card header */}
      <div className={`relative h-32 bg-gradient-to-br ${note.grad}
                flex items-center justify-center flex-shrink-0`}>
        <span className="text-5xl select-none">{note.emoji}</span>

        {/* Difficulty tag */}
        <span className={`absolute top-3 left-3 px-2.5 py-0.5 rounded-full
                    text-[10px] font-black tracking-wide ${note.tagColor}`}>
          {note.difficulty}
        </span>
        ...
        <span className="absolute top-3 right-3 px-2.5 py-0.5 rounded-full
                    ...
                    backdrop-blur-sm">
          {formatIcon[note.format]} {note.format}
        </span>
...
function Notes() {
  ...
  const filtered = useMemo(() => {
    return notes.filter((n) => {
      ...
      const matchFormat = format === "All Formats" || n.format === format;
      return matchSearch && matchCategory && matchFormat;
    });
  }, [notes, search, category, format]);

  useEffect(() => { setCurrent(1); }, [search, category, format]);
    ...
    <div className="min-h-screen bg-white dark:bg-slate-950 text-gray-900 dark:text-gray-100">
      ...
      <div ref={topRef} className="max-w-6xl mx-auto px-6 py-10">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {(search || category !== "All" || format !== "All Formats") && (
            <button
              onClick={() => { setSearch(""); setCategory("All"); setFormat("All Formats"); }}
        ...
        {paginated.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <h3 className="font-black text-lg mb-2">No notes found</h3>
