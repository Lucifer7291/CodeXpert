...
import data from "../data/Roadmaps.json";
...
function RoadmapCard({ roadmap, index }) {
  return (
    ...
    >
      {/* Card header */}
      <div className={`relative h-32 bg-gradient-to-br ${roadmap.grad}
                flex items-center justify-center flex-shrink-0`}>
        <span className="text-5xl select-none">{roadmap.emoji}</span>

        {/* Level tag */}
        <span className={`absolute top-3 left-3 px-2.5 py-0.5 rounded-full
                    text-[10px] font-black tracking-wide ${roadmap.tagColor}`}>
          {roadmap.level}
        </span>
      ...
      <div className="flex flex-col flex-1 p-5">
        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          {roadmap.milestones.map((m, i) => (
            <span key={m} className="flex items-center gap-1 px-2 py-0.5
...
function Roadmaps() {
  ...
  const { page, filters, roadmaps } = data;
  ...
  const filtered = useMemo(() => {
    return roadmaps.filter((r) => {
      ...
      const matchDuration = duration === "All Durations" || r.duration === duration;
      return matchSearch && matchCategory && matchDuration;
    });
  }, [roadmaps, search, category, duration]);

  useEffect(() => { setCurrent(1); }, [search, category, duration]);
    ...
    <div className="min-h-screen bg-white dark:bg-slate-950 text-gray-900 dark:text-gray-100">
      ...
      <div ref={topRef} className="max-w-6xl mx-auto px-6 py-10">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {(search || category !== "All" || duration !== "All Durations") && (
            <button
              onClick={() => { setSearch(""); setCategory("All"); setDuration("All Durations"); }}
        ...
        {paginated.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <h3 className="font-black text-lg mb-2">No roadmaps found</h3>
